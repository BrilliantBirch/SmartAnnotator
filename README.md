...
| **模型预加载**    | 模型设置确认后后台加载与预热（进度弹窗实时显示），模型缓存复用，后续标注秒级启动                                   |
| **视频标注**      | 视频抽帧标注（帧间隔/差异阈值可调），含预览播放器与损坏视频保护                                                    |
| **检测类别选择**  | 模型加载后自动解析元数据类别，支持复选/全选/取消全选，按类别过滤推理结果                                           |
| **OCR 仅识别**    | 跳过文本检测阶段，仅对已标注 shape 区域执行文本识别并回写结果，提升处理效率                                        |
| **感知区（ROI）** | 画布绘制/管理感知区（虚线渲染 + `ROI{id}` 标识、列表 Dock 联动），按感知区裁剪导出聚焦数据集（跨边界标注自动截断） |
...
> - **旧版兼容**：无 `ROI` 字段的 JSON 正常加载（视为无感知区）；ROI 为空时不写该字段，老文件保存后不新增键
> - ROI 不进入 `shapes` 数组，不参与 LabelMe ↔ YOLO 转换与标签扫描统计
...
- 产品版本：`2.3.0`（唯一来源 `smart_annotator/__init__.py` 的 `__version__`，发版时仅改此一处）
...
```
画布绘制 / 矩形标注转换 → Canvas._rois（撤销栈快照含 shapes + rois）
    → 保存：labelme_io.set_document_rois → JSON 顶层 ROI 字段（为空则不写键）
    → 导出：core/roi_export.export_image_rois（裁剪图片 + 标注平移/截断）
        ├─ 手动：感知区菜单 → RoiExportDialog → RoiExportWorker（模态进度，可中止）
        └─ 自动：保存成功后挂钩导出（RenderConfig.roi_auto_export 持久化开关）
    → 产物：<工作路径>/ROI/{原图名}_ROI{id}{原扩展名} + 同名 .json（不含 ROI 字段）
```

> 新增 ROI 动作时须同时补齐 `_ACTION_DEFS`（app.py）与 `DEFAULT_SHORTCUTS`（config.py），
> 两者键集合必须一致（快捷键解析与自定义快捷键对话框依赖该约束，已由单测守护）。
> 自动标注覆盖写盘时经 `annotator._restore_rois` 保留磁盘既有 ROI，重新标注不会丢失感知区。
